...
const tables={students:'students',tutors:'tutors',schedules:'schedules',attendance:'attendance',payments:'payments'};
...
app.get('/api/reports/summary',auth,async(req,res)=>{try{const r=await q(`select (select count(*) from students where status='Aktif') students,(select count(*) from students where status not in ('Aktif')) registrations,(select count(*) from tutors) tutors,(select count(*) from schedules) schedules,(select count(*) from attendance) attendance,(select count(*) from payments) payments,(select coalesce(sum(amount),0) from payments where status='Lunas') paid_total`);res.json(r[0])}catch(e){res.status(500).json({error:e.message})}});
